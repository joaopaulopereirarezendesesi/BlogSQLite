...
class Router {
  ...
  async run(req, res) {
    try {
      ...
      if (routeType === "api") {
        ...
        await this.handlePageRequest(controllerOrPage, req, res);
      } else {
  ...
  parseUrl(req) {
    try {
      ...
      utils.jsonResponse(
        res,
        { error: `Erro ao executar ação: ${e.message}` },
        500
      );
    }
  }
  ...
  async handleApiRequest(controllerName, action, param, res) {
    try {
      const controllerPath = path.join(
        ...
        "..",
        "controllers",
        this.sanitizeControllerName(controllerName) + ".js"
      );
      let ControllerClass;
      ...
      try {
        ...
        return utils.jsonResponse(
          res,
          { error: `Controlador inválido: ${controllerName}` },
          404
        );
      }
      ...
      if (this.isValidAction(controller, this.sanitizeActionName(action))) {
        await this.callAction(
          controller,
          this.sanitizeActionName(action),
          param,
          res
        );
      } else {
        return utils.jsonResponse(
          res,
          { error: `Método inválido: ${action} em ${controllerName}` },
          404
        );
      }
    } catch (e) {
  ...
  async handlePageRequest(pageName, req, res) {
    try {
      const pageRenderer = new PageRenderer();
      await pageRenderer.render(pageName, req, res);
    } catch (e) {
